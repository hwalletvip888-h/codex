...
function summarizeDay(day, ledger) {
  ...
  return {
    ...
    cycleSummaries: (ledger.cycleSummaries || []).filter((item) => dayKey(item.generatedAt) === day).length,
    latestCycleSummary: (ledger.cycleSummaries || [])[0] || null,
